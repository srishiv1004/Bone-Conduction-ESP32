...
// --- FORWARD DECLARATION ---
// This tells the compiler the function exists later in the file
...
static void i2s_adc_task(void *arg)
{
    ...
    // Free memory if task ever exits
    free(i2s_read_buff);
...
void loop() {
  // We must call poll() to keep the websocket alive and process events
  if(client.available()) {
      client.poll();
  }
}
